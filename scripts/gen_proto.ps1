...
$GeneratePython = -not $DartOnly
$GenerateDart = -not $PythonOnly

if ($PythonOnly -and $DartOnly) {
    throw "-PythonOnly and -DartOnly cannot be used together."
}
...
if ($GenerateDart) {
    if (Get-Command protoc -ErrorAction SilentlyContinue) {
        ...
        throw "protoc not found. Install the Protocol Buffers compiler."
    }

    if (-not (Get-Command dart -ErrorAction SilentlyContinue)) {
        throw "dart command not found. Install Flutter and add its bin directory to PATH. See https://docs.flutter.dev/install/add-to-path"
    }
...
if ($GeneratePython) {
    if ($DryCheck) {
        Write-DryCheck "Would ensure output directory exists: $PY_OUT"
    } else {
...
if ($GenerateDart) {
    if ($DryCheck) {
        Write-DryCheck "Would ensure output directory exists: $DART_OUT"
    } else {
...
if ($GenerateDart) {
    ...
    if ($DryCheck) {
        ...
        & $script:ProtocBin `
            ...
            $PROTO_FILE
        if ($LASTEXITCODE -ne 0) {
            throw "Dart gRPC generation failed with exit code $LASTEXITCODE"
        }
    }
}
